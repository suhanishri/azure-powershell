...
foreach ($moduleName in $allModules) {
    ...
    if ($hasRawData) {
        ...
        ConvertTo-Json $moduleExecReport -Compress -Depth 3 | Out-File -FilePath ([System.IO.Path]::Combine($cvgResultsDir, "$moduleName.json")) -Encoding utf8 -NoNewline -Force
    ...
    Write-Host "##[section]`"$moduleName`" total cmdlets: $totalCmdletsCount"
    Write-Host "##[section]`"$moduleName`" tested cmdlets: $totalExecCmdletsCount"
